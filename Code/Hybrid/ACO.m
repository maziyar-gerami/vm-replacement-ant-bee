function [WA, numberOfeachVm] = ACO(CreatedVMs)
rng(14);
%% Problem Definition
...
eta=ones(nVms,nVms);      % Heuristic Information Matrix


tau=tau0*ones(nVms, nVms);         % Phromone Matrix
...
for i=1:MaxIt
    ...
    for j=1:nVms    
                ...
                tau(j, update.Ants(1).Position(j))= tau(j, update.Ants(1).Position(j))+ rho*(update.Ants(1).Wastage);
    ...
    for k=2:20
        ...
        for j=1:nVms    
                ...
                tau(j, update.Ants(k).Position(j))= tau(j, update.Ants(k).Position(j))+ update.Ants(k).Wastage;
